...
power_results <- map_dfr(
  sample_sizes,
  ~ run_power_sim(simulated_data, n_sample = .x, n_sim = 10),
  .id = "sample_step"
) |> 
...
```{r message=FALSE, warning=FALSE, fig.height = 6, fig.width = 8, fig.cap = "Power curves for detecting a positive correlation (gray line) and for concluding statistical equivalence to zero (red line) across different sample sizes. Dashed and dotted lines represent 80% and 90% power thresholds, respectively. Vertical dotted lines represents the sample size where power reaches $1 - \\beta$ = 0.9."}
# Parameters
eq_bounds <- 0.15
rho_pop <- 0.30
alpha <- 0.05
